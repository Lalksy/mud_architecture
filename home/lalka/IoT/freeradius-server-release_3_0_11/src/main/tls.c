...
#ifdef GEN_URI
	case GEN_URI:
	vp = fr_pair_make(talloc_ctx, certs, cert_attr_names[FR_TLS_SAN_URI][lookup],(char *) ASN1_STRING_data(name->d.uniformResourceIdentifier), T_OP_SET);
	Uri[sizeof(subject) - 1] = '\0';
	strcpy(Uri,( (char *) ASN1_STRING_data(name->d.uniformResourceIdentifier)));
	strcpy(urI,Uri);
	break;
#endif //GEN_URI
